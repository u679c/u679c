<p align="center">
 <img src="https://capsule-render.vercel.app/api?type=waving&height=180&text=Hi,%20I'm%20\u679c&fontAlign=50&fontAlignY=35&color=C8EB55" />
</p>
  
## 关于我

<p>
  <img src="https://img.shields.io/badge/\u679c-C8EB55"/>
  <br/>
  是果在unicode中的转义！那正是我名字中的一部分😊
</p>  

从 麦当劳餐饮呈递、甜品站产品制作，到上了大学开始 对接甲方成为了**产品经理**兼**项目经理**兼**前后端开发**、大四 跟随校企合作前往北京长亭科技干**技术支持**、再到现在回过头继续做**前端开发**，这一路大大小小见证了自己太多时刻，回想起来还真是 **“颇有感触”**～

关于敲代码？我从小学开始接触scratch，初中使用MIT制作安卓app，同时期在达内童程童美青少年编程上课，安卓Java开发（说实话根本没记住当时讲的java代码哈哈，但是很多概念和思想已经进入脑子！）、js（ES5）、python、c++信息学奥赛 balabala..

参与国际级、国家级、省级、校级大大小小赛事若干。

从小就想当程序员，现在也算是实现了梦想了哈哈哈，敲代码真的很幸福！

### 都说，说当爱好成为工作，那便不再会是爱好了，可是我觉得虽然工作中会有乏味的地方在，但爱好永远是爱好！我享受创造产品的过程，热爱一个个点子一个个想法落地的感受。

在我的公开仓库中有许多生活当中想到“如果有 可以再便利一点！”而去实现的内容，大大提高了自己的便利。

---

我觉得应该不断认识到自己的问题，哪怕没有解决办法，哪怕不说出来，只有知道自己缺点，下次触发了就会意识到从而慢慢就“迭代”自己～

当然啦，缺点是社会定义的，要权衡“个性化”和“体制化”之间的关系，否则要么变成社会中的“疯子”，要么变成“行尸走肉🧟”毫无生机

---

- 网络工程专业 / 前端、后端、运维
- 正在做：个人项目、开源工具、学习笔记、工作！！
- 目标：持续输出项目、持续迭代技术
 <br/>
 Fix the bugsssss? No! this is the start of a new mini-saga of discovery~
 <br/>
 <br/>
 
  [个人简介](http://47.109.137.27:8800/)
  
 <br/>
 <img src="https://img.shields.io/badge/About-u679c-black"/>
   
   
## 技术、工具
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Vue-4FC08D?logo=vue.js&logoColor=white"/>
  <img src="https://img.shields.io/badge/Django-092E20?logo=django&logoColor=white"/>
  <img src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white"/>
  <img src="https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black"/>
  <img src="https://img.shields.io/badge/Vim-019733?logo=vim&logoColor=white"/>
  <img src="https://img.shields.io/badge/React-019733?logo=react&logoColor=white"/>
</p>
<!--
## 特色项目
 <p>
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=u679c&repo=CNTS" />
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=u679c&repo=v-pet" />
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=u679c&repo=feiyi_game" />
</p> 

## GitHub Stats
<p>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=u679c&layout=compact" />
</p>
-->
